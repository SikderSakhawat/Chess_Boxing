...
import ChessEngine.board.Move.MoveFactory;
import ChessEngine.gui.Table.MoveLog;
...
public class PGNUtil {
    ...
    public static void persistPGNFile(final File pgnFile) throws IOException {
        ...
        try (final BufferedReader br = new BufferedReader(new FileReader(pgnFile))) {
            String line;
            PGNTag.TagsBuilder tagsBuilder = new PGNTag.TagsBuilder();
                ...
                if (!line.isEmpty()) {
                    if (isTag(line)) {
                        ...
                        if(!gameText.isEmpty() && gameText.length() > 80) {
                            ...
                            if(game.isValid()) {
                                SQLgamePersist.get().persistGame(game);
                                validCount++;
                            }
                        }
                        gameTextBuilder = new StringBuilder();
                        tagsBuilder = new PGNTag.TagsBuilder();
                    }
                    else {
    ...
    public static List<String> processMoveText(final String gameText) throws ParsingException {
    ...
    public static Move createMove(final Board board,
        ...
        if(kingSideCastleMatcher.matches()) {
            ...
            destinationCoordinate = BoardUtility.INSTANCE.getCoordinateAtPosition(destinationSquare);
    ...
    private static int deriveCurrentCoordinate(final Board board,
        ...
        final int destinationCoordinate =  BoardUtility.INSTANCE.getCoordinateAtPosition(destinationSquare);
        for (final Move move : board.currentPlayer().getLegalMoves()) {
    ...
    private static int extractFurther(final List<Move> candidateMoves,
        ...
        for (final Move move : currentCandidates) {
            final String pos = BoardUtility.INSTANCE.getPositionAtCoordinate(move.getCurrentCoordinate());
