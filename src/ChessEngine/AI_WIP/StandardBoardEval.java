...
import static ChessEngine.AI_WIP.KingAnalyze.*;
import static ChessEngine.piece.Piece.PieceType.BISHOP;

public final class StandardBoardEval
        implements BoardEval {
    ...
    private static int pawnStructure(final Player player) {
        return PawnAnalyze.get().pawnStructureScore(player);
    }

    private static int kingSafety(final Player player) {
        final KingDistance kingDistance = KingAnalyze.get().calculateKingTropism(player);
