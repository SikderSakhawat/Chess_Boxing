...
import ChessEngine.board.MoveTransition;
...
import static ChessEngine.board.BoardUtility.mvvlva;

public class StockAB extends Observable implements MoveStrat {
    ...
    private enum MoveSorter {
        ...
        EXPENSIVE {
            ...
            Collection<Move> sort(final Collection<Move> moves) {
                return Ordering.from((Comparator<Move>) (move1, move2) -> ComparisonChain.start()
                        .compareTrueFirst(BoardUtility.kingThreat(move1), BoardUtility.kingThreat(move2))
                        .compareTrueFirst(move1.isCastlingMove(), move2.isCastlingMove())
                        .compare(mvvlva(move2), mvvlva(move1))
                        .result()).immutableSortedCopy(moves);
            }
    ...
    public StockAB(final int searchDepth) {
    ...
    private int max(final Board board,
                    ...
                    final int highest,
                    final int lowest) {
        if (depth == 0 || BoardUtility.isEndGame(board)) {
    ...
    private int calculateQuiescenceDepth(final Board toBoard,
        ...
        if(depth == 1 && this.quiescenceCount < MAX_QUIESCENCE) {
            ...
            for(final Move move: BoardUtility.lastNMoves(toBoard, 2)) {
                if(move.isAttack()) {
                    activityMeasure += 1;
                }
            }
